...
import cv2
import numpy as np
from components.hand_keypoints.handpose_x import handpose_x_model,draw_bd_handpose_c, draw_mask_handpose
import math
...
def handpose_track_keypoints21_pipeline(img,hands_dict,hands_click_dict,track_index,algo_img = None,handpose_model = None,gesture_model = None,
    ...
    if algo_img is not None:
        ...
        for idx,id_ in enumerate(sorted(hands_dict.keys(), key=lambda x:x, reverse=False)):
            ...
            cv2.putText(img, 'LABEL {}'.format(tags[str(pre_tag)]), (int(x_min + 2), int(y_min + 15)), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        (0, 0, 255), 3)
